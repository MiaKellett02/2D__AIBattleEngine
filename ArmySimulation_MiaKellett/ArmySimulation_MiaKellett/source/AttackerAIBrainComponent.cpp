...
Vector2 AttackerAIBrainComponent::HandleDodgingAlliesMovement()
{
...
Vector2 AttackerAIBrainComponent::HandleDodgingEnemies()
{
	return Vector2();
}

Vector2 AttackerAIBrainComponent::MoveTowardsClosestEnemy(Entity* a_enemy)
{
	if (a_enemy == nullptr) {
		return Vector2(0, 0);
	}
	...
	//Figure out movement.
	Vector2 moveDir = a_enemy->GetPosition() - m_ownerEntity->GetPosition();
	if (moveDir.SqrMagnitude() < ((m_ownerEntity->GetSize().x / 2) * (m_ownerEntity->GetSize().x / 2))) {
		return Vector2(0, 0);
	}

	//Return the final momvement.
